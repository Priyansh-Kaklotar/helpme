...
export async function GET() {
    ...
    return NextResponse.json(reviews, { status: 200, headers: { "Content-Type": "application/json" } });
}
...
export async function POST(req) {
    try {
        ...
        if(userType ==="Customer"){
            ...
            return NextResponse.json({ success: true, message: "Review added successfully" }, {status:200});
        }
        else {
            throw new Error("Service Provider cannot rate other Service provider")
        }
    } catch (error) {
        return NextResponse.json({success : false ,message: error.message}, {status:400}); // error
    }
}